...
public class GuiGraphics {
	...
	private void applyScissor(@Nullable ScreenRectangle screenRectangle) {
		...
		if (screenRectangle != null) {
			...
			double e = screenRectangle.left() * d;
			double f = i - screenRectangle.bottom() * d;
			double g = screenRectangle.width() * d;
			double h = screenRectangle.height() * d;
	...
	public void fill(RenderType renderType, int i, int j, int k, int l, int m, int n) {
		...
		float f = ARGB32.alpha(n) / 255.0F;
		float g = ARGB32.red(n) / 255.0F;
		float h = ARGB32.green(n) / 255.0F;
		float p = ARGB32.blue(n) / 255.0F;
		VertexConsumer vertexConsumer = this.bufferSource.getBuffer(renderType);
		vertexConsumer.vertex(matrix4f, i, j, m).color(g, h, p, f).endVertex();
		vertexConsumer.vertex(matrix4f, i, l, m).color(g, h, p, f).endVertex();
		vertexConsumer.vertex(matrix4f, k, l, m).color(g, h, p, f).endVertex();
		vertexConsumer.vertex(matrix4f, k, j, m).color(g, h, p, f).endVertex();
	...
	private void fillGradient(VertexConsumer vertexConsumer, int i, int j, int k, int l, int m, int n, int o) {
		...
		float h = ARGB32.green(n) / 255.0F;
		float p = ARGB32.blue(n) / 255.0F;
		float q = ARGB32.alpha(o) / 255.0F;
		float r = ARGB32.red(o) / 255.0F;
		float s = ARGB32.green(o) / 255.0F;
		float t = ARGB32.blue(o) / 255.0F;
		Matrix4f matrix4f = this.pose.last().pose();
		vertexConsumer.vertex(matrix4f, i, j, m).color(g, h, p, f).endVertex();
		vertexConsumer.vertex(matrix4f, i, l, m).color(r, s, t, q).endVertex();
		vertexConsumer.vertex(matrix4f, k, l, m).color(r, s, t, q).endVertex();
		vertexConsumer.vertex(matrix4f, k, j, m).color(g, h, p, f).endVertex();
	}
	...
	public int drawString(Font font, @Nullable String string, int i, int j, int k, boolean bl) {
		if (string == null) {
			...
			int l = font.drawInBatch(string, i, j, k, bl, this.pose.last().pose(), this.bufferSource, Font.DisplayMode.NORMAL, 0, 15728880, font.isBidirectional());
			this.flushIfUnmanaged();
			return l;
		}
	}
	...
	public void blit(ResourceLocation resourceLocation, int i, int j, int k, int l, int m, int n) {
		this.blit(resourceLocation, i, j, 0, k, l, m, n, 256, 256);
	}
	...
	void blit(ResourceLocation resourceLocation, int i, int j, int k, int l, int m, int n, int o, float f, float g, int p, int q) {
		this.innerBlit(resourceLocation, i, j, k, l, m, (f + 0.0F) / p, (f + n) / p, (g + 0.0F) / q, (g + o) / q);
	}
	...
	public void renderComponentHoverEffect(Font font, @Nullable Style style, int i, int j) {
		if (style != null && style.getHoverEvent() != null) {
			...
			if (itemStackInfo != null) {
				...
				EntityTooltipInfo entityTooltipInfo = (EntityTooltipInfo)hoverEvent.getValue(Action.SHOW_ENTITY);
				if (entityTooltipInfo != null) {
					if (this.minecraft.options.advancedItemTooltips) {
						this.renderComponentTooltip(font, entityTooltipInfo.getTooltipLines(), i, j);
					}
				} else {
					Component component = (Component)hoverEvent.getValue(Action.SHOW_TEXT);
					if (component != null) {
